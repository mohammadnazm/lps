...
/* School Admin: gender + grade breakdown */
...
.no-price-note a{color:var(--accent)}
...
<?php if ($isGeneral): ?>
    <?php
    // ============== GENERAL ADMIN: schools + per-school totals only ==============
    $schools = getSchools(false);
    $totalStudents = adminScalar("SELECT COUNT(*) FROM students WHERE st_statue='بەردەوام'");
    $totalTeachers = adminScalar("SELECT COUNT(*) FROM teachers");
    $totalAttendance = adminScalar("SELECT COUNT(DISTINCT student_id) FROM attendance WHERE date='$today'");
    ?>
    <div class="hero">
        <div><h1>General Administration</h1><p>Every school on the platform, at a glance.</p></div>
    ...
    <div class="school-grid">
        <?php foreach ($schools as $s):
            $code = $s['school_code'];
            $count = adminScalar("SELECT COUNT(*) FROM students WHERE school_scope='" . $conn->real_escape_string($code) . "' AND st_statue='بەردەوام'");
        ?>
...
<?php else: ?>
    <?php
    // ============== SCHOOL ADMIN: their own school's numbers only ==============
    $school = getSchoolByCode($scope);
    $schoolName = $school['school_name'] ?? $scope;

    $totalStudents = adminScalar("SELECT COUNT(*) FROM students WHERE school_scope='" . $conn->real_escape_string($scope) . "' AND st_statue='بەردەوام'");
    $totalTeachers = adminScalar("SELECT COUNT(*) FROM teachers WHERE school_scope='" . $conn->real_escape_string($scope) . "'");
    $totalAttendance = adminScalar("SELECT COUNT(DISTINCT a.student_id) FROM attendance a INNER JOIN students s ON s.id=a.student_id WHERE a.date='$today' AND s.school_scope='" . $conn->real_escape_string($scope) . "'");
    ...
    ?>
    <div class="hero">
        <div><h1><?=htmlspecialchars($schoolName)?></h1><p>Your school's students, at a glance.</p></div>
    ...
    <div class="section-title">Students by Class <a href="class_prices.php">Manage Class Prices →</a></div>
    <?php if (!$anyPriceConfigured): ?>
    <div class="no-price-note">No class prices are set yet, so discounts show as 0 everywhere. Set each grade's standard price on the <a href="class_prices.php">Class Prices</a> page.</div>
    <?php endif; ?>
