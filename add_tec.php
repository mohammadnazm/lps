...
if (isset($_POST["u_add"])) {
    ...
    insertTeacherss($_POST["uname"], $_POST["upass"], $_POST["u_role"], $_POST["school_scope"] ?? null);
    echo "</div>";
...
<div style="display: flex;flex-direction: column;flex-wrap: wrap;gap:5%;">
        ...
        <form action="" method="post">
            <div style="display: flex;gap:10px;flex-wrap: wrap;">
                ...
                <select class="form-control card shadow-sm" name="u_role" required>
                    ...
                    <?php
                    $stdList = getDh("subjects");
                    if ($stdList->num_rows > 0) {
                        while ($row = $stdList->fetch_assoc()) {
                            echo "<option value='" . $row['sb_name'] . "'>" . $row['sb_name'] . "</option>";
                        }
                    }
                    ?>
                </select>
        ...
        <table class="table table-striped">
            <thead>
                <tr>
                    ...
                    <th style="background-color:grey; text-align: center;">Subject</th>
            ...
            <tbody>
                ...
                if ($stdList->num_rows > 0) {
                    while ($row = $stdList->fetch_assoc()) {
                        ...
                        echo "<td>{$counter}</td>";
                        echo "<td style='text-align:center'>{$row['id']} - {$row['t_name']}</td>";
                        echo "<td style='text-align:center'>••••••••</td>";
                        echo "<td style='text-align:center'><button class='btn btn-warning btn-sm' style='border-radius:80px'>{$row['t_sub']}</button></td>";
                        echo "<td style='text-align:center'><span class='badge bg-info text-dark'>{$row['school_scope']}</span></td>";
                        echo "<td style='text-align:center'>
                                <a class='btn btn-secondary btn-sm' role='button' href='reset_password.php?type=teacher&id=" . $row['id'] . "' onclick=\"window.open(this.href,'PopupWindow','width=420,height=420,scrollbars=yes'); return false;\">Reset Password</a>
                                <a class='btn btn-danger btn-sm' role='button' href='add_tec.php?did=" . $row['id'] . "' onclick=\"return confirm('Are you sure you want to delete this?');\"'>Delete</a>
                              </td>";
                        echo "</tr style='text-align:center'>";
                        $counter++;
                        $totcou = $counter;
    ...
    <h6>Total Number Of Users : <?php if ($totcou == 0) {
                                    $totcou = 1;
                                }
                                echo $totcou - 1; ?></h6>
