...
if (isset($_POST["u_add"])) {
    ...
    insertTeacherss($_POST["uname"], $_POST["upass"], $_POST["u_role"]);
    echo "</div>";
            ...
            <tbody>
                ...
                if ($stdList->num_rows > 0) {
                    while ($row = $stdList->fetch_assoc()) {
                        ...
                        echo "<td style='text-align:center'>{$row['t_pass']}</td>";
                        echo "<td style='text-align:center'><button class='btn btn-warning btn-sm' style='border-radius:80px'>{$row['t_sub']}</button></td>";
                        echo "<td style='text-align:center'><a class='btn btn-danger btn-sm' role='button' href='add_tec.php?did=" . $row['id'] . "' onclick=\"return confirm('Are you sure you want to delete this?');\"'>Delete</a></td>";
                        echo "</tr style='text-align:center'>";
                        $counter++;
                        $totcou = $counter;
                    }
