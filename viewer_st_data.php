...
function getDhySearch($table)
{
    ...
    $sql = "SELECT * FROM `$table` WHERE " . implode(" AND ", $conditions) . " AND " . genderSql() . " AND " . schoolSql("school_scope") . " ORDER BY st_name ASC";
...
?>
<?php
if (isset($_GET['did']) && isset($_SESSION['loginadmin']) && $_SESSION['loginadmin'] === true) {
    $id = intval($_GET['did']);
    $getImg = mysqli_query($conn, "SELECT st_img,st_id_file FROM students WHERE id = $id AND " . schoolSql('school_scope') . "");
