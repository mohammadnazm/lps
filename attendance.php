        ...
        foreach ($ids as $key => $sid) {
            ...
            $check = mysqli_query(
                ...
                "SELECT id FROM attendance 
                 WHERE student_id='$sid' AND date='$date' AND EXISTS (SELECT 1 FROM students s WHERE s.id=attendance.student_id AND " . schoolSql('s.school_scope') . ")"
            );

            if (mysqli_num_rows($check) == 0 && scopedStudentExists($sid)) {
...
function getDhAttendance($table, $class, $group)
{
    ...
    $gender = mysqli_real_escape_string($conn, (string)($_SESSION['useraccess'] ?? 'All'));
    $genderSql = ($gender !== '' && strtolower($gender) !== 'all') ? " AND st_gender='$gender'" : '';
    $sql = "SELECT * FROM `$table` WHERE st_class='$class' AND st_group='$group'" . $genderSql . " AND " . schoolSql('school_scope') . " ORDER BY st_name ASC";
    ...
    <form action="" method="post">
        ...
        <div style="display:flex; gap:10px;">
            ...
            <select class="form-control card shadow-sm"
                ...
                <?php for ($g=1; $g<=12; $g++) echo "<option value='$g'>$g</option>"; ?>
