<?php
ob_start(); // ✅ FIX HEADER ERROR

include "admin_header.php";

/* ================= SESSION MESSAGE ================= */
if (isset($_SESSION['msg5'])) {
    $msg = $_SESSION['msg5'];
    unset($_SESSION['msg5']);
}

/* ================= INSERT TEACHER ================= */
if (isset($_POST["sub_st"])) {

    $filename = handleSecureUpload('st_img', 'teachers_img', ['jpg', 'jpeg', 'png', 'gif'], true) ?? '';

    // ✅ MULTIPLE CLASSES SUPPORT
    $classes = isset($_POST['st_gp']) ? (array)$_POST['st_gp'] : [];

    foreach ($classes as $class) {

        if ($class != "") {
            insertTeachers(
                $filename,
                $_POST['st_name'],
                $_POST['st_m_name'],
                $class,
                $_POST['school_scope'] ?? null
            );
        }
    }

    $_SESSION['msg5'] = "Teacher added successfully!";

    header("Location: " . $_SERVER['PHP_SELF']);
    exit();
}

/* ================= DISABLE CLASS ================= */
if (isset($_POST["cl_class"])) {

    if (!empty($_POST['disable_class'])) {

        insertClosedClass(
            $_POST['disable_class'],
            "disabled",
            $_POST['school_scope'] ?? null
        );

        $_SESSION['msg5'] = "Class Disabled successfully!";
    }

    header("Location: " . $_SERVER['PHP_SELF']);
    exit();
}
?>

<?php
if (isset($_GET['did'])) {
    DeleteData("lozanstaff", $_GET['did']);

    $_SESSION['msg5'] = "Teacher Deleted successfully!";
    header("Location: " . $_SERVER['PHP_SELF']);
    exit();
}

if (isset($_GET['did2'])) {
    DeleteData("staffclasscon", $_GET['did2']);

    $_SESSION['msg5'] = "Class Enabled successfully!";
    header("Location: " . $_SERVER['PHP_SELF']);
    exit();
}
?>

<style>
/* ===== PAGE LAYOUT ===== */
html, body {
    height: 100%;
    margin: 0;
    padding: 0;
    font-family: 'Segoe UI', sans-serif;
    background: #0f172a;
    color: #f1f5f9;
    overflow: hidden;
}

.page-container {
    display: flex;
    flex-direction: column;
    height: 100vh;
    padding: 20px;
    gap: 20px;
    overflow-y: auto;
}

.page-title {
    text-align: center;
    font-size: 24px;
    font-weight: 700;
}

/* FORM */
.student-form-card {
    background: linear-gradient(145deg, #1e293b, #0f172a);
    padding: 25px;
    border-radius: 16px;
    box-shadow: 0 6px 18px rgba(0,0,0,0.4);
}

/* INPUTS */
.student-form-card input,
.student-form-card select {
    background: #1e293b !important;
    color: #f1f5f9 !important;
    border: 1px solid rgba(255,255,255,0.15) !important;
    border-radius: 10px !important;
    padding: 10px !important;
}

/* FLEX */
.student-form-card > form > div {
    display: flex;
    flex-wrap: wrap;
    gap: 20px;
}
</style>

<div class="page-container">

    <div class="page-title">Lozan Staff Form</div>

    <?php if (isset($msg)) { ?>
        <div class="alert alert-success"><?php echo $msg; ?></div>
    <?php } ?>

    <div class="student-form-card">

        <form method="post" enctype="multipart/form-data">

            <div>
                <input type="text" name="st_name" placeholder="T Name" required>
                <input type="text" name="st_m_name" placeholder="Education">
                <input type="file" name="st_img">

                <!-- MULTIPLE CLASS SELECT -->
                <select name="school_scope" required style="width:15%;">
                    <option value="" disabled selected>School</option>
                    <?php if (canSeeAllSchools()) { ?>
                        <?php foreach (getSchools(true) as $school) { echo "<option value='" . htmlspecialchars($school['school_code'], ENT_QUOTES) . "'>" . htmlspecialchars($school['school_name']) . "</option>"; } ?>
                    <?php } else { ?><option value="<?php echo htmlspecialchars(currentSchoolScope()); ?>" selected><?php echo htmlspecialchars(currentSchoolScope()); ?></option><?php } ?>
                </select>
                <select name="st_gp[]" multiple style="width: 15%; height:120px;text-align: center;">
                    <?php
                    foreach (['Kids'] as $specialGrade) { foreach (range('A','Z') as $letter) echo "<option value='{$specialGrade}-{$letter}'>{$specialGrade}-{$letter}</option>"; }
                    for ($grade=1; $grade<=12; $grade++) {
                        foreach (range('A', 'Z') as $letter) {
                            echo "<option value='{$grade}-$letter'>{$grade}-$letter</option>";
                        }
                    }
                    ?>
                </select>

                <!-- SINGLE CLASS DISABLE -->
                <select name="disable_class" style="width: 15%;">
                    <option value="">Disable Class</option>
                    <?php
                    for ($grade=1; $grade<=12; $grade++) {
                        foreach (range('A', 'Z') as $letter) {
                            echo "<option value='{$grade}-$letter'>{$grade}-$letter</option>";
                        }
                    }
                    ?>
                </select>

                <button type="submit" name="cl_class" class="btn btn-danger col-2">
                    Disable
                </button>
            </div>

            <div style="width:100%; margin-top:20px;">
                <button type="submit" name="sub_st" class="btn btn-success col-12">
                    Add Staff
                </button>
            </div>

        </form>

    </div>

    <hr>

    <!-- ================= TEACHERS TABLE (UNCHANGED) ================= -->
    <div class="scrollableBox">
        <table class="table table-striped">
            <thead>
                <tr>
                    <th>#</th>
                    <th>Username</th>
                    <th>Education</th>
                    <th>Class</th>
                    <th>School</th><th>Action</th>
                </tr>
            </thead>
            <tbody>
                <?php
                $counter = 1;
                $stdList = getDh("lozanstaff");

                if ($stdList->num_rows > 0) {
                    while ($row = $stdList->fetch_assoc()) {
                        echo "<tr style='text-align:center'>";
                        echo "<td>{$counter}</td>";
                        echo "<td>{$row['name']}</td>";
                        echo "<td>{$row['education']}</td>";
                        echo "<td><button class='btn btn-info btn-sm'>{$row['class']}</button></td>";
                        echo "<td><span class='badge bg-info text-dark'>{$row['school_scope']}</span></td>";
echo "<td>
    <a class='btn btn-danger btn-sm'
       href='?did={$row['id']}'
       onclick=\"return confirm('Are you sure you want to delete this teacher?');\">
       Delete
    </a>
</td>";                        echo "</tr>";
                        $counter++;
                    }
                }
                ?>
            </tbody>
        </table>
    </div>

    <!-- ================= CLASS TABLE (UNCHANGED) ================= -->
    <div class="scrollableBox">
        <table class="table table-striped">
            <thead>
                <tr>
                    <th>#</th>
                    <th>Class</th>
                    <th>Status</th>
                    <th>Action</th>
                </tr>
            </thead>
            <tbody>
                <?php
                $counter = 1;
                $stdList = getDh("staffclasscon");

                if ($stdList->num_rows > 0) {
                    while ($row = $stdList->fetch_assoc()) {
                        echo "<tr style='text-align:center'>";
                        echo "<td>{$counter}</td>";
                        echo "<td><button class='btn btn-info btn-sm'>{$row['class_name']}</button></td>";
                        echo "<td>{$row['class_status']}</td>";
echo "<td>
    <a class='btn btn-danger btn-sm'
       href='?did2={$row['id']}'
       onclick=\"return confirm('Are you sure you want to delete this class?');\">
       Delete
    </a>
</td>";                        echo "</tr>";
                        $counter++;
                    }
                }
                ?>
            </tbody>
        </table>
    </div>

</div>

<?php include "admin_footer.php"; ?>
<?php ob_end_flush(); ?>