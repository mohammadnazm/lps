...
if (isset($_POST["log"])) {
    ...
    } elseif (tooManyFailedLogins($username)) {

        $error = "Too many failed login attempts. Please wait " . LOGIN_LOCKOUT_MINUTES . " minutes and try again.";
    ...
    if ($result->num_rows == 1) {
        ...
        if (verifyAndUpgradePassword($password, (string)$row['u_pass'], 'users', 'id', $row['id'], 'u_pass')) {

            $loggedIn = true;
            clearFailedLogins($username);
            session_regenerate_id(true);

            $_SESSION['school_scope'] = strtoupper($row['school_scope'] ?? 'LOZAN');
            $_SESSION['user_role'] = $row['u_role'];
            $_SESSION['username'] = $row['u_name'];
            switch ($row['u_role']) {

                case "GeneralAdmin":
                    // Dedicated master account for the whole platform.
                    // It can create/manage schools and school administrators.
                    $_SESSION['loginadmin'] = true;
                    $_SESSION['general_admin'] = true;
                    $_SESSION['userid'] = $row['id'];
                    $_SESSION['school_scope'] = 'ALL';
    ...
    if ($teacher->num_rows == 1) {
        ...
        if (verifyAndUpgradePassword($password, (string)$row['t_pass'], 'teachers', 'id', $row['id'], 't_pass')) {
    ...
    // ================= STUDENTS TABLE =================
    // Students log in with their ID number, which also doubles as a printed/displayed
    // identifier elsewhere in the app (profile, ID card) — it isn't hashed like a real
    // password, so this stays a constant-time direct compare rather than password_verify.
    if (!$loggedIn) {
    ...
    if ($student->num_rows == 1) {
        ...
        if ($row['st_id_number'] !== null && hash_equals((string)$row['st_id_number'], $password)) {
