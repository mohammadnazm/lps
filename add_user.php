...
if (isset($_POST["u_add"])) {
    $role = $_POST["u_role"] ?? '';
    if ($role === 'Admin' && !isGeneralAdmin()) {
        die('Only General Administration can create School Admin accounts.');
    }
    if (!isGeneralAdmin()) {
        $_POST["school_scope"] = currentSchoolScope();
    }
    echo "<div class='alert alert-success' role='alert'>";
    insertUsers($_POST["uname"], $_POST["upass"], $role, $_POST["u_for_role"], $_POST["school_scope"] ?? null);
    echo "</div>";
        ...
        <form action="" method="post">
            <div style="display: flex;gap:10px;flex-wrap: wrap;">
                ...
                <select class="form-control card shadow-sm" name="u_role" required>
                    ...
                    <?php if (isGeneralAdmin()) { ?><option value="Admin">School Admin</option><?php } ?>
                ...
                <select class="form-control card shadow-sm" name="school_scope" required>
                    <option value="" disabled selected>School Access</option>
                    <?php if (isGeneralAdmin()) { ?>
                        ...
                        <?php } ?>
                    <?php } else { ?>
                        <option value="<?=htmlspecialchars(currentSchoolScope(), ENT_QUOTES)?>" selected><?=htmlspecialchars(schoolName(currentSchoolScope()))?></option>
                    <?php } ?>
                </select>
        ...
        <table class="table table-striped">
            <thead>
                <tr>
                    ...
                    <th style="background-color:grey; text-align: center;">School</th><th style="background-color:grey; text-align: center;">Action</th>
                </tr>
            </thead>
            <tbody>
                <?php
                ...
                if ($stdList->num_rows > 0) {
                    while ($row = $stdList->fetch_assoc()) {
                        ...
                        echo "<td style='text-align:center'><span class='badge bg-info text-dark'>{$row['school_scope']}</span></td>";
                        echo "<td style='text-align:center'>
                                <a class='btn btn-secondary btn-sm' role='button' href='reset_password.php?type=user&id=" . $row['id'] . "' onclick=\"window.open(this.href,'PopupWindow','width=420,height=420,scrollbars=yes'); return false;\">Reset Password</a>
                                <a class='btn btn-danger btn-sm' role='button' href='add_user.php?did=" . $row['id'] . "' onclick=\"return confirm('Are you sure you want to delete this?');\"'>Delete</a>
                              </td>";
                        echo "</tr style='text-align:center'>";
                        $counter++;
                        $totcou = $counter;
