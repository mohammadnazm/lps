...
$id = intval($_GET['did']);
$newIdFile = handleSecureUpload('st_id_file_in', 'id_data', ['pdf', 'jpg', 'jpeg', 'png'], false);

if ($newIdFile === null) {
    echo "<div class='alert alert-danger' role='alert'>Upload failed — please choose a PDF, JPG, or PNG file under 8MB.</div>";
} else {
$getImg = mysqli_query($conn, "SELECT st_id_file FROM students WHERE id = $id AND " . schoolSql('school_scope') . "");
$row = mysqli_fetch_assoc($getImg);

if (!empty($row['st_id_file'])) {

    $oldFile = $row['st_id_file'];
    $extensions = ['jpg', 'jpeg', 'png', 'gif', 'avif', 'pdf'];
    ...
    echo "<div class='alert alert-success' role='alert'>";
    updateImageAndFilesOfSt($_GET['did'], $newIdFile);
    echo "</div>";
}
}
?>
...
$id = intval($_GET['did']);
$newPhoto = handleSecureUpload('st_p_img', 'st_image', ['jpg', 'jpeg', 'png', 'gif'], true);

if ($newPhoto === null) {
    echo "<div class='alert alert-danger' role='alert'>Upload failed — please choose a JPG, PNG, or GIF image under 8MB.</div>";
} else {
$getImg = mysqli_query($conn, "SELECT st_img FROM students WHERE id = $id AND " . schoolSql('school_scope') . "");
$row = mysqli_fetch_assoc($getImg);
...
if (!empty($row['st_img'])) {
    ...
    echo "<div class='alert alert-success' role='alert'>";
    updateImageAndFilesOfSt2($_GET['did'], $newPhoto);
    echo "</div>";
}
}
?>
                ...
                <div method="post" style="display:flex;flex-direction:column;gap:6px">
                    <div style="display:flex;align-items:center;gap:10px;">
                        <select class="form-control card shadow-sm" name="st_class">
                            ...
                            <option value="Kids" <?php if ($stclass == "Kids") echo "selected"; ?>>Kids</option>
                            <?php for ($g=1; $g<=12; $g++) echo "<option value='$g' ".($stclass==(string)$g?'selected':'').">$g</option>"; ?>
                        </select>
                    ...
                    <div style="display:flex;align-items:center;gap:10px;">
                        <select class="form-control card shadow-sm" name="st_fak">
                            ...
                            <option value="تایبەت" <?php if ($stfaculty == "تایبەت") echo "selected"; ?>>تایبەت</option>
                        </select>
